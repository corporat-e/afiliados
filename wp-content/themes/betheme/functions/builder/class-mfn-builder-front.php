  ...
  class Mfn_Builder_Front {
    ...
    public function show( $items = false ){
  		...
  		if( isset( $items ) && is_array( $items ) ){
  			$mfn_items = $items;
  		} else if( empty( $_GET['preview'] ) && empty( $_GET['mfn-preview'] ) ){
              ...
              continue;
  				...
  				if ($section['attr']['bg_image'] && ($section_bg_attr[2] == 'fixed')) {
  					if (! key_exists(4, $section_bg_attr) || $section_bg_attr[4] != 'still') {
  						...
  						if( !$_GET['visual'] || empty( $_GET['visual'] ) || !isset( $items ) ){
  						// parallax
                ...
                ksort( $section['wraps'] );
  							foreach ($section['wraps'] as $w => $wrap) {
  								...
  								if( ( ! empty( $_GET['visual'] ) && $_GET['visual'] == 'iframe' ) || ( isset( $items ) && is_array( $items ) ) ){
  									echo '<div data-order="'. $w .'"  data-uid="'. $wrap['uid'] .'" data-size="'.$wrap['size'].'" class="wrap vb-item mcb-wrap '. $wrap_class .' clearfix" style="'. $wrap_style .'" '. $parallax .' '. $wrap_data .'>';
                        ...
                        ksort($wrap['items']);
  											...
  											foreach ($wrap['items'] as $i => $item) {
													...
													if (method_exists('Mfn_Builder_Items', $type)) {
  													...
  													// output -----
  													if( ( ! empty($_GET['visual']) && $_GET['visual'] == 'iframe' ) || ( isset( $items ) && is_array( $items ) ) ){
  														echo '<div data-order="'. $i .'"  data-uid="'. $item['uid'] .'" data-size="'.$item['size'].'" class="column vb-item mcb-column '. $item_class .'">';
